...
for ($i = 0; $i < AUTHORS_COUNT; $i++) {
    ...
    $author_name = $i > 0 ? strtolower($faker->name()) : 'Vincent Justin';
    $author_slug = $slugify->slugify($author_name);
    $author_avatar = $faker->imageUrl(128, 128, 'people', true, $author_name);
    $author_email = $i > 0 ? $faker->unique()->safeEmail : 'justin.vincent@student.hepl.be';
    $author_password = password_hash('Azerty', PASSWORD_DEFAULT);
    $pdo->exec(<<<SQL
